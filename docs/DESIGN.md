...
3. 精选收录（Featured）：读取 `featuredOrder: 1` 至 `6` 的真实条目卡片。
4. 近期更新（Updates）：非 Note 条目的时间线沉淀。
5. Notes：文章横条列表，支持浮卡预览与独立长文页。
6. 探索（Explore）：全量条目探索引擎，支持搜索、类型过滤、标签过滤与瀑布流。
7. 关于与页脚（About & Footer）：关于本园的哲学原则与黑底页脚。
